                ...
                <AlertDialogContent>
                    ...
                    <AlertDialogBody>
                        <Box width='100%' key={generalWasteItemDetail.id}>
                            <Text fontWeight='bold'>{generalWasteItemDetail.description}</Text>
                            <Text my={4}>Reason: {generalWasteItemDetail.reason}</Text>
                            <Text>Suggestion: {generalWasteItemDetail.suggestion}</Text>
                        </Box>
                    </AlertDialogBody>

                    <AlertDialogFooter>
                        <HStack>
                            {hasPrevGeneralWasteItems && <Button colorScheme='green' variant='outline' onClick={() => setDialogItemIndex(dialogItemIndex - 1)} ml={3}>
                                Previous
                            </Button>}
                            {hasNextGeneralWasteItems && <Button colorScheme='green' onClick={() => setDialogItemIndex(dialogItemIndex + 1)} ml={3}>
                                Next
                            </Button>}
                            {!hasNextGeneralWasteItems && <Button colorScheme='green' onClick={navigateToGeolocation} ml={3}>
                                Confirm
                            </Button>}
                        </HStack>
                    </AlertDialogFooter>
