  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 50, marginBottom: 10 }}>
          Your location is <strong>{Address}. </strong>         <Button onClick={() => { setGeolocation(true); setLocation(false); }}>Dispose Items Yourself Instead</Button>
    </div>
    <div>
        Your items are:
        <br/>
        {items.map((item) => (
            <p key={item.id}>{item.description}<br/></p>
        ...
        {orgs.map((org) => (
          <Accordion defaultIndex={[0]} allowMultiple>
            <AccordionItem key={org.id}>
              ...
              <AccordionPanel pb={2}>
                <b>Website: </b>
                <a style={{ color: "#14828A" }} href={org.url}>
                  {org.website}
                </a>
              </AccordionPanel>
              <AccordionPanel pb={2}>
    ...
    </div>) : <div><div>Enter your home address:</div><AsyncSelect
        value={Address}
        isSearchable
